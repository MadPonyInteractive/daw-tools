...
class PianoScene(QGraphicsScene):
    ...
    def drawPiano(self):
        keys_width = self.piano_width - self.padding
        labels = ('B','Bb','A','Ab','G','Gb','F','E','Eb','D','Db','C')
        black_notes = (2,4,6,9,11)
        piano_label = QFont()
        piano_label.setPointSize(6)
        self.container = QGraphicsRectItem(0, 0, keys_width, self.piano_height)
        self.container.setPos(0, self.header_height)
        self.addItem(self.container)
        key = PianoKeyItem(keys_width, self.note_height, self.container)
...
class PianoView(QGraphicsView):
    def __init__(self, parent=None):
        QGraphicsView.__init__(self)
        self.scene = PianoScene(parent)
